    ...
    Context "When running the function" {
        BeforeAll {
            ...
            function Start-TestProcess {
                return Start-Job -ScriptBlock { Start-Sleep -Seconds 2; $proc = Start-Process -FilePath $args[0] -PassThru; Start-Sleep 2; Stop-Process  $proc } -ArgumentList "$PSScriptRoot\..\simple.exe"
            }
        }
