...
"@ | Out-File -FilePath $xml -Encoding UTF8
        ...
        It 'Should update the node' {
            Update-NxtXmlNode -FilePath $xml -NodePath '/catalog' -InnerText 'Test' -Attributes @{'metadata'='test'} | Should -BeNullOrEmpty
        ...
        It 'Should only update the filtered nodes' {
            Update-NxtXmlNode -FilePath $xml -NodePath '/catalog/product/catalog_item' -InnerText 'Test' -FilterAttributes @{"gender" = "Women's"} | Should -BeNullOrEmpty
            $xmlDoc = [xml](Get-Content -Path $xml -Raw)
