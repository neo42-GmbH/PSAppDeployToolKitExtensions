...
function Complete-NxtPackageInstallation {
	...
	Process {
		...
		foreach ($oldAppFolder in $((Get-ChildItem -Path (Get-Item -Path $App).Parent.FullName | Where-Object Name -ne (Get-Item -Path $App).Name).FullName)) {
			...
			Execute-Process -Path powershell.exe -Parameters "-File `"$oldAppFolder\Clean-Neo42AppFolder.ps1`"" -WorkingDirectory "$oldAppFolder" -NoWait -PassThru | Out-Null
		}
	...
	Process {
		...
		try {
			if ($null -ne (Get-SchedulerTask -ContinueOnError $true | Select-Object -Property 'TaskName' | Where-Object {
				...
				Execute-Process -Path $exeSchTasks -Parameters "/Delete /TN $schTaskBlockedAppsName /F" -PassThru | Out-Null
			}
		}
		catch {
					...
					(Get-NxtRegisteredPackage -ProductGUID $ProductGUID).PackageGUID | Where-Object {
						...
						if ($false -eq ([string]::IsNullOrEmpty($assignedPackageGUIDAppPath))) {
							if ($true -eq (Test-Path -Path "$assignedPackageGUIDAppPath")) {
								...
								Start-Sleep -Seconds 1
								Execute-Process -Path powershell.exe -Parameters "-File `"$assignedPackageGUIDAppPath\Clean-Neo42AppFolder.ps1`"" -WorkingDirectory "$assignedPackageGUIDAppPath" -NoWait -PassThru |Out-Null
								$removalCounter += 1
							}
							else {
